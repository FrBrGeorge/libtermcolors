...
int tc_color_sequence(const char *filename, const char *name, char **sequence) {
    ...
    *sequence = NULL;
    ...
    if (!f) {
        debug_print("Could not open colorscheme file: %s", filename);
        return TERMCOLORS_NOT_FOUND;
    }

    debug_print("Reading colorscheme from: %s", filename);
    ...
    while ((read = getline(&line, &len, f)) != -1) {
        ...
        if (name_len == strlen(name) && strncmp(name_start, name, name_len) == 0) {
            ...
            *sequence = strdup(p);
            debug_print("Found color '%s' with raw sequence: %s", name, p);
            found = 1;
            break;
        }
    }
